...
CreditTable::CreditTable(size_t n)
{
    tab.resize(n);
}
...
double CreditTable::GetMonthlyPayment(size_t idx) const {
    return tab[idx][0];
}
...
double CreditTable::GetInterestPayment(size_t idx) const {
    return tab[idx][2];
}
...
double CreditTable::GetBalanceOwed(size_t idx) const {
    return tab[idx][3];
}
